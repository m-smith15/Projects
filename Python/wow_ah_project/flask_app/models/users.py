...
# import tables from mydb (schema)
class User:
    ...
    def create_user(cls, user_data):
        ...
        return connectToMySQL('mydb').query_db(query, user_data)
    
    @classmethod
    def validate_login(cls, user_data):

        query = "SELECT * FROM users WHERE email = %(email)s"
        results = connectToMySQL('mydb').query_db(query, user_data)
        if len(results) < 1:
            return False
    ...
    def existing_user_check(email_check):
        ...
        results = connectToMySQL('mydb').query_db(query, email_check)
